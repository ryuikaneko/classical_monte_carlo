...
*/
  ...
  fprintf(fp_para,"# 12-16: MA-MB\n");
  fprintf(fp_para,"# 17-21: |MA-MB|\n");
  fprintf(fp_para,"# 22-26: MA+MB\n");
  fprintf(fp_para,"# 27-31: sqrt(MA^2+MB^2)\n");
/*
  fprintf(fp_para,"# 1:     T\n");
  fprintf(fp_para,"# 2-6:   E\n");
  fprintf(fp_para,"# 7-11:  M\n");
...
*/
  ...
  for(i=0; i<N_smp; i++){
    ...
    for(m=0; m<N_t; m++){/* loop for each temperature */
      ...
      d_temp[0] = Eadd[m];
      d_temp[1] = Madd[m][0];
      d_temp[2] = Madd[m][1]-Madd[m][2];
      d_temp[3] = fabs(Madd[m][1]-Madd[m][2]);
      d_temp[4] = Madd[m][1]+Madd[m][2];
      d_temp[5] = sqrt(Madd[m][1]*Madd[m][1]+Madd[m][2]*Madd[m][2]);
/*
...
*/
  ...
  for(m=0; m<N_t; m++){/* loop for each temperature */
    ...
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][13]/T[m]);// X of MA-MB
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][18]/T[m]);// X of |MA-MB|
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][23]/T[m]);// X of MA+MB
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][28]/T[m]);// X of sqrt(MA^2+MB^2)
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][11]);// X' of MA-MB
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][16]);// X' of |MA-MB|
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][21]);// X' of MA+MB
    fprintf(fp_phys_chosen,"%13.6e ",PhysQuant[m][26]);// X' of sqrt(MA^2+MB^2)
