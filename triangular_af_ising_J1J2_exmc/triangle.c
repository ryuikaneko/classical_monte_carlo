  ...
  for(i=0; i<size_L; i++){
    ...
    for(j=0; j<size_L; j++){
      ...
      tmp_ene +=
        ...
        - J2 * spins[m][i][j]
          * (spins[m][im][jp] + spins[m][ip][jpp] + spins[m][ipp][jp]);
    }
  }
  ene[m] = tmp_ene;
  return 0;
...
)
{
  ...
  egap2 = 2 * spins[m][i][j]
    *(spins[m][im][jp] + spins[m][ip][jm]
    + spins[m][ip][jpp] + spins[m][im][jmm]
    + spins[m][ipp][jp] + spins[m][imm][jm]);
  /* metropolis method */
  if(dsfmt_genrand_close_open(&dsfmt) <= BoltzFac[list_temp[m]][egap1+12][egap2+12]){
    spins[m][i][j] *= -1;
  }
  return 0;
}
...
)
{
  ...
  if(dsfmt_genrand_close_open(&dsfmt) <= BoltzFac[list_temp[m]][egap1+12][egap2+12]){
    spins[m][i][j] *= -1;
    ene[m] += J1 * egap1 + J2 * egap2;
    mag[m][0] += 2 * spins[m][i][j];
    mag[m][1+(i+j)%3] += 2 * spins[m][i][j];
  }
  return 0;
}
...
//  seed=1234567;
  ...
  for(m=0; m<N_t; m++){
    for(i=-12; i<=12; i++){
      for(j=-12; j<=12; j++){
        BoltzFac[m][i+12][j+12] = exp(-(J1*i+J2*j)/T[m]);
      }
    }
  }
  ...
  for(i=0; i<N_equ; i++){
    for(m=0; m<N_t-1; m++){/* loop for each temperature */
      ...
      calc_energy(size_L,list_conf[m],J1,J2,spins,Eadd);
      calc_energy(size_L,list_conf[m+1],J1,J2,spins,Eadd);
      exchange_states(size_L,m,J1,J2,T,list_temp,list_conf,spins,Eadd);
    }
    for(m=0; m<N_t; m++){/* loop for each temperature */
      /* m: parameter of environment */
      for(j=0; j<Ns; j++){
  ...
  for(m=0; m<N_t; m++){/* loop for each temperature */
    /* m: parameter of environment */
    calc_energy(size_L,m,J1,J2,spins,Eadd);
  ...
  for(i=0; i<N_smp; i++){
    ...
    for(m=0; m<N_t; m++){/* loop for each temperature */
      /* m: parameter of environment */
      for(j=0; j<Ns; j++){
        one_flip_ene_mag(size_L,m,J1,J2,BoltzFac,list_temp,spins,Eadd,Madd);
      }
    }
...
//// index in {0,1,2,...,N} (N+1 components)
      ...
      i_temp1 = (int)(0.5*(N_hist_mag*(d_temp[2]/Ns)/R_hist_mag+N_hist_mag+1));
      i_temp2 = (int)(0.5*(N_hist_mag*(d_temp[3]/Ns)/R_hist_mag+N_hist_mag+1));
